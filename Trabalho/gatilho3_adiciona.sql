...
create view if not exists horarioSemanaEmCausa as select Dia_folga,Data_segunda,turnoDiurno,turnoNoturno from Horario,NEW,Medico where (cast(strftime('%J',NEW.Data) as int) - cast(strftime('%J',Horario.Data_segunda) as int)) < 7 and Medico.idFuncionario = Consulta.Medico and Horario.idFuncionario = Medico.idFuncionario;
...
drop trigger if exists consultaDisponivel;
